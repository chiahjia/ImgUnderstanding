...
#path = "/Users/chiah/Documents/csc420/A2/University_College.jpg"
...
def corner_detector(gray_img, rgb_img, window_size, stddev):
	...
	print("shape is ", rgb_img.shape)

	for r in range (rows):
		print("r is ", r)
		for c in range (cols):
			if M_11[r,c] > 40000 and M_22[r,c] > 40000:
				new_img[r,c] = (255, 0, 0)

	plt.imshow(new_img)
	plt.show()

def draw_circle(ori_img, corners, rows, cols):
	print("inside draw")
	for r in range (rows):
		for c in range (cols):
			print(r,c)
			if corners[r,c] > 0:
				rr, cc = draw_circle(r,c, 2, ori_img.shape)
				ori_img[rr,cc,:] = (0,0,255)
...
corner_detector(gray_img, rgb_img, 5, 10)
